...
function normalize(data, partial=false){
  ...
  if(!partial || data.role!==undefined) result.role=validation.enumValue(data.role,'Role',['DRIVER','CONDUCTOR','MANAGER','SUPPORT'],!partial);
...
async function assign({tripId,staffId,assignmentRole}){
  ...
  const {rows}=await pool.query(`INSERT INTO trip_staff_assignments(trip_id,staff_id,assignment_role) VALUES($1::uuid,$2::uuid,$3) ON CONFLICT(trip_id,staff_id,assignment_role) DO NOTHING RETURNING *`,[tripId,staffId,assignmentRole]);
