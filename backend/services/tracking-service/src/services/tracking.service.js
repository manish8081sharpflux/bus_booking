...
class TrackingService {
  ...
  async experience(tripId, auth) {
    ...
    const { rows } = await db.query(`SELECT t.id,t.service_number,t.departure_at,t.arrival_at,t.status,t.declared_delay_minutes,t.declared_delay_reason,t.declared_delay_at,t.breakdown_status,r.source_city,r.destination_city,b.name bus,o.display_name operator,COALESCE(JSON_AGG(JSON_BUILD_OBJECT('id',ts.id,'order',ts.stop_order,'city',ts.city,'name',ts.location_name,'address',ts.address,'lat',COALESCE(ts.latitude,rs.latitude),'lng',COALESCE(ts.longitude,rs.longitude),'scheduledAt',CASE WHEN ts.stop_order=1 THEN t.departure_at WHEN ts.stop_order=(SELECT MAX(x.stop_order) FROM trip_stops x WHERE x.trip_id=t.id) THEN t.arrival_at ELSE ts.scheduled_at END) ORDER BY ts.stop_order),'[]') stops FROM trips t JOIN routes r ON r.id=t.route_id JOIN buses b ON b.id=t.bus_id JOIN operators o ON o.id=t.operator_id JOIN trip_stops ts ON ts.trip_id=t.id LEFT JOIN route_stops rs ON rs.route_id=t.route_id AND rs.stop_order=ts.stop_order WHERE t.id=$1::uuid GROUP BY t.id,r.id,b.id,o.id`, [tripId]);
